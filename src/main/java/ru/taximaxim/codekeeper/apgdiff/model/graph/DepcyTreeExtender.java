...
public class DepcyTreeExtender {
     ...
     * @return
     */
    private TreeElement getEmptyTreeChild(TreeElement tree){
    ...
    private void copyFilteredToNew(TreeElement filtered, TreeElement copy) {
        for (TreeElement child : filtered.getChildren()){
            PgStatement sasasa = child.getPgStatement(depcySource.getDb()/*dbSource*/);
            DiffSide side = child.getSide();
            if (sasasa != null && shouldBeDeleted.contains(sasasa)){
                side = DiffSide.LEFT;
                TreeElement childCopy = new TreeElement(child.getName(), child.getType(), child.getContainerType(), side);
                shouldBeDeleted.remove(sasasa);
                addChildrenToLeft(child, childCopy);
     ...
     * @param childCopy
     */
    private void addChildrenToLeft(TreeElement child, TreeElement childCopy) {
        for(TreeElement chichi : child.getChildren()){
            TreeElement chichiCopy = new TreeElement(chichi.getName(), chichi.getType(), chichi.getContainerType(), DiffSide.LEFT);
            childCopy.addChild(chichiCopy);
            addChildrenToLeft(chichi, chichiCopy);
        }
    }
