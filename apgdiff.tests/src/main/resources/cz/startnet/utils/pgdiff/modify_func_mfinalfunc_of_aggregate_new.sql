...
CREATE OR REPLACE FUNCTION public.mode_bool_final(integer[], boolean, integer, text) RETURNS boolean
    LANGUAGE sql
    AS $_$
...
CREATE FUNCTION public.mode_m_final(bigint[], boolean, integer, text) RETURNS bytea
    LANGUAGE sql
    AS $_$
SELECT CASE WHEN ( $1[1] = 0 AND $1[2] = 0 )
THEN NULL
