...
-- DEPCY: This AGGREGATE depends on the FUNCTION: public.mode_m_inv(bigint[], boolean, text)

DROP AGGREGATE public.mode11_params(boolean, integer, text ORDER BY boolean, text);

DROP FUNCTION public.mode_m_inv(bigint[], boolean, text);

-- DEPCY: This FUNCTION is a dependency of AGGREGATE: public.mode11_params(boolean, integer, text, boolean, text)

CREATE OR REPLACE FUNCTION public.mode_m_inv(bigint[], boolean, text) RETURNS integer[]
    LANGUAGE sql
    AS $_$
SELECT CASE 1 > 2
WHEN TRUE THEN
