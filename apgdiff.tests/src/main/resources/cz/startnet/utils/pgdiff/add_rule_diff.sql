...
CREATE RULE on_insert AS
    ON INSERT TO t1 DO INSTEAD ( INSERT INTO user_data (id, email, created) VALUES (new.id, new.email, new.created);
 INSERT INTO t1(c1) DEFAULT VALUES;
);

CREATE RULE on_update AS
    ON UPDATE TO t1 DO INSTEAD  UPDATE user_data SET id = new.id, email = new.email, created = new.created WHERE (user_data.id = old.id);
