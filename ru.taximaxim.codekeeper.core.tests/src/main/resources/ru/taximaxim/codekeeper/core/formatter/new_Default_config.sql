...
exception
  when others then
    get stacked diagnostics _sqlstate = returned_sqlstate, _message_text = message_text, _context = pg_exception_context;
    ...
    select array_agg(public.id) into id_total from
      (select id from public.events where _update > 30 and last_update > c_update and id_name = any(_name)
      UNION select id from public.events where _update > 5 and last_update > c__update  and c_name = any (name_5m)) alias_;
    ...
    /*WHILE LOOP*/
    while count(_people) > 0 loop
      _people = test.get_people(_people, _name);
      _people = _people - 1;
    end loop;

    /*FOR LOOP*/
    for _id_test in select id from public.people where age > 20
    loop
      update test.workers set status = "work" where id = _id_test;
    end loop;

    /*FOREACH LOOP*/
    foreach _id in array params
    loop
      insert into public.test (id, _name, _date) values (_id, name_1, public.getDate()) returning public.test.id into _id;
    end loop;
