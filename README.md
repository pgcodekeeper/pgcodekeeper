...
Build requires Java (JDK) 8+ and Apache Maven 3.6+.
...
- `apgdiff.tests` - majority of unit and functional tests.
...
- `cz.startnet.utils.pgdiff.schema.meta` - simplified SQL object classes for database structure serialization.
- `cz.startnet.utils.pgdiff.schema.meta` (resources) - serialized descriptions of PostgreSQL's system objects (built using JdbcSystemLoader).
...
- `cz.startnet.utils.pgdiff.parsers.antlr.statements` - processor classes that create schema objects based on parser-read data. Notable classes:
  - `ParserAbstract` - common base class for all processors.
- `cz.startnet.utils.pgdiff.parsers.antlr.expr` - expression analysis classes: find dependencies in expressions and infer expression types for overloaded function call resolution.
- `cz.startnet.utils.pgdiff.parsers.antlr.expr.launcher` - support for parallel expression analysis.
- `cz.startnet.utils.pgdiff.formatter` - SQL and pl/pgsql code formatters.
- `cz.startnet.utils.pgdiff` - util and general-purpose classes. Notable classes:
  - `PgDiff` - entry point for database schema diff operations.
- `ru.taximaxim.codekeeper.apgdiff.model.difftree` - classes representing and creating an object diff tree. Notable classes:
...
- `ru.taximaxim.codekeeper.apgdiff.model.exporter` - classes that save and update the project files on disk. Notable classes:
...
- `cz.startnet.utils.pgdiff` - `PgDiffTest`, `MsDiffTest` - these test cases load old and new database schemas, generate a migration script and compare it to the expected diff file.
- `cz.startnet.utils.pgdiff.depcies` - tests here work similarly to simple diff tests above, except the concept of "user selection" is added. Migration script is built only with objects in `usr` files as starting points, other objects must be added to the script via dependency mechanism.
- `cz.startnet.utils.pgdiff.loader` - `PgAntlrLoaderTest`, `MsAntlrLoaderTest` - these tests load simple schemas from files and compare loaded PgDatabase objects with predefined ones.
- `cz.startnet.utils.pgdiff.parsers` - these tests simply parse test pieces of code (taken from PostgreSQL source) to verify parser validity.
- `cz.startnet.utils.pgdiff.parsers.antlr.expr` - these tests verify expression analysis and type inference mechanism.
- `ru.taximaxim.codekeeper.apgdiff.model.exporter` - these test update exported project directories and verify which files have actually been changed.
