...
ALTER TYPE public.user_code OWNER TO test;
...
ALTER SEQUENCE public.emp_id_seq OWNER TO test;
...
ALTER FUNCTION public.emp_stamp() OWNER TO test;
...
ALTER FUNCTION public.increment(i integer) OWNER TO test;
...
ALTER TABLE public.emp OWNER TO test;
...
CREATE VIEW public.emp_view AS
    SELECT empname,
    last_date,
    public.increment(salary) AS salary,
    code
   FROM public.emp;

ALTER VIEW public.emp_view OWNER TO test;
...
CREATE FOREIGN DATA WRAPPER wrap
 ...
 ALTER FOREIGN DATA WRAPPER wrap OWNER TO test;
...
ALTER FOREIGN TABLE public.ft OWNER TO test;
...
ALTER TABLE public.test
	ADD CONSTRAINT exc_con EXCLUDE USING btree (c1 WITH =);

-- tests constraint comments

ALTER TABLE public.test ADD CONSTRAINT test_pk_c1 PRIMARY KEY (c1) WITH (FILLFACTOR = '10');

COMMENT ON CONSTRAINT test_pk_c1 on public.test is 'constraint comment';

-- tests grant all with grant option

REVOKE ALL ON TABLE public.test FROM test;
GRANT ALL ON TABLE public.test TO test WITH GRANT OPTION;
...
CREATE POLICY p1 ON public.test AS RESTRICTIVE FOR SELECT TO test USING (true);
CREATE POLICY p2 ON public.test AS PERMISSIVE FOR INSERT TO test WITH CHECK ((1 > 0));
CREATE POLICY p3 ON public.test TO test USING (true) WITH CHECK ((1 > 0));
...
ALTER SERVER "asdashSA/sdag" OWNER TO test;
...
ALTER TEXT SEARCH DICTIONARY public.testdictionary OWNER TO test;
...
ALTER TEXT SEARCH CONFIGURATION public.testconfig OWNER TO test;
...
ALTER AGGREGATE public.avg(double precision) OWNER TO test;
...
ALTER TABLE public.seqtable OWNER TO test;
...
ALTER FOREIGN TABLE public.child_xz OWNER TO test;
-- tests mat view
...
ALTER TABLE public.mv1 OWNER TO test;
...
ALTER FUNCTION public.pr(boolean, boolean) OWNER TO test;
...
ALTER OPERATOR public.=== (boolean, boolean) OWNER TO test;
...
ALTER FUNCTION public.user_code(integer) OWNER TO test;
...
ALTER SCHEMA country OWNER TO test;

CREATE SCHEMA worker;

ALTER SCHEMA worker OWNER TO test;

CREATE TABLE country.city (
    id integer
);

ALTER TABLE country.city OWNER TO test;

CREATE TABLE worker.people (
    fio text
);

ALTER TABLE worker.people OWNER TO test;
...
ALTER FUNCTION country.get_city() OWNER TO test;
...
ALTER FUNCTION worker.get_changes() OWNER TO test;

CREATE SCHEMA ignore1;

ALTER SCHEMA ignore1 OWNER TO test;

CREATE TABLE ignore1.testschema (
    fio text
);

ALTER TABLE ignore1.testschema OWNER TO test;
...
ALTER FUNCTION ignore1.get_schema() OWNER TO test;

CREATE SCHEMA ignoreI4vrw;

ALTER SCHEMA ignoreI4vrw OWNER TO test;

CREATE TABLE ignoreI4vrw.testschema2 (
    fio text
);

ALTER TABLE ignoreI4vrw.testschema2 OWNER TO test;

CREATE SCHEMA ignore;

ALTER SCHEMA ignore OWNER TO test;
