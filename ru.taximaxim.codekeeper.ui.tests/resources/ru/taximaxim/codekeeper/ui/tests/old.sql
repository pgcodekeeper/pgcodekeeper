...
CREATE FUNCTION fun2() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    BEGIN
...
CREATE FUNCTION proc(i integer) RETURNS void
    LANGUAGE plpgsql
    AS $$
    BEGIN
...
CREATE FUNCTION proc(i integer, t timestamp without time zone) RETURNS void
    LANGUAGE plpgsql
    AS $$
    BEGIN
...
ALTER TABLE t2_c1_seq OWNER TO galiev_mr;

--
-- Name: t2_c1_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: galiev_mr
--

ALTER SEQUENCE t2_c1_seq OWNED BY t2.c1;
...
ALTER TABLE test_table_2 OWNER TO galiev_mr;
...
ALTER TABLE ONLY t2 ALTER COLUMN c1 SET DEFAULT nextval('t2_c1_seq'::regclass);
...
CREATE INDEX t2_c5_idx ON t2 USING btree (c5);
...
CREATE INDEX t5_c1_idx ON t5 USING btree (c1);
...
CREATE INDEX test_table_c1_idx ON test_table USING btree (c1);
...
CREATE INDEX test_table_c2_idx ON test_table USING btree (c2);
...
CREATE TRIGGER t1_trigger AFTER DELETE ON t1 FOR EACH ROW EXECUTE PROCEDURE fun1();
...
CREATE TRIGGER t2_trigger AFTER DELETE ON t2 FOR EACH ROW EXECUTE PROCEDURE fun2();
