...
- Добавлено удаление файла со ссылками при ошибке десериализации.

### Изменено

- При сохранении файла проекта больше не перечитывается весь проект.
...
- Удалена страница настроек `Тип соединения`. Цвет соединения теперь задаётся напрямую в редакторе подключения к БД.
- Обновлены библиотечные зависимости.

### Исправлено

- Исправлено логирование ошибок в самостоятельных сборках pgCodeKeeper-а.
- Исправлен парсинг данных о таблице в `Мастере генерации тестовых данных`.

## [14.7.0] - 2026-06-30

### Добавлено

- В `Мастер сравнения` добавлены кнопки `Поменять местами` и `Сформировать скрипт миграции`. Кнопка `Finish` больше не формирует скрипт миграции.
...
- Исправлено поведение текстового поля при использовании мастера создания новых объектов.
...
- Исправлена ошибка при генерации скрипта в `Мастере сравнения`.
- Исправлена ошибка в сборщике проекта.

### Удалено

- Удалена возможность добавления дополнительных зависимостей для текущего сравнения.
...
- Добавлена возможность указания кластера для таблиц в ClickHouse проектах (`Project -> Properties -> pgCodeKeeper -> Обновление БД -> Добавлять ON CLUSERT для таблиц (эксперементально)`).
- Добавлена настройка для формирования скрипта миграции по актуальной версии базы данных (`Window -> Preferences -> pgCodeKeeper -> Обновления БД -> Использовать синтаксис текущей версии базы данных для генерации скрипта миграции.`) (на текущий момент нет изменений синтаксиса, они появятся позднее). По умолчанию используется синтаксис, характерный для минимальной поддерживаемой версии.
...
- Исправлена ошибка при создании ClickHouse схемы через мастер создания объектов.
- Исправлена ошибка отображения сообщения при экспорте подключений на macOS.
...
- Добавлено название группы в автогенерируемое имя соединения.
- Добавлена настройка выбора режимов загрузки баз данных (`Window -> Preferences -> pgCodeKeeper -> Использовать параллельную загрузку БД`).
- Добавлена локализация пропущенных строк.

### Изменено

- Обновлена версия драйвера clickhouse-jdbc. Теперь все подключения к ClickHouse должны включать обязательный параметр `password`.
...
- Исправлено отображение коллекций ClickHouse в результатах запросов.
...
- Исправлена ошибка создания объекта SEQUENCE в файле таблицы через мастер создания новых объектов.
- Исправлена ошибка, возникающая при пакетном переносе изменений в MS SQL.
...
- Удалена возможность редактирования ini-файла из плагина. Теперь настройка количества потоков парсера недоступна из окна настроек. Обновлено диалоговое окно, которое появляется при нехватке оперативной памяти: теперь оно показывает справку вместо изменения конфигурации.
...
- Исправлена ошибка с алгоритмом шифрования.

## [10.15.0] - 2025-07-29

### Исправлено

- Исправлены правила парсера.
...
- Добавлено шифрование файла с настройками подключений. После сохранения настроек, предыдущие версии не смогут прочитать данные настройки.
...
- Исправлена загрузка схемы из директории в `Мастере сравнения`.
...
- Исправлена ошибка в работе `Мастера сравнения`.
