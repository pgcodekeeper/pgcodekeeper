...
- Добавлено поле для выбора типа базы данных из списка поддерживаемых БД.
- Добавлена поддержа синтаксиса ALTER INDEX в MS SQL.

### Изменено

- Изменен механизм работы с объектами CONSTRAINT и INDEX: объекты теперь хранятся не в виде строки, а в виде отдельных полей, что позволяет избежать лишних различий в ключевых словах и пробелах.
