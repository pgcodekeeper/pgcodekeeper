...
- Параметр `--ms-sql` заменен на параметр `--db-type` с указанием типа БД. На текущий момент поддерживается 2 значения: PG (значение по-умолчанию) и MS. Параметр `--ms-sql` помечен как устаревший и будет удален в будущих версиях. Все флаги с выбором MSSQL базы заменены на комбобокс с выбором типа БД.
