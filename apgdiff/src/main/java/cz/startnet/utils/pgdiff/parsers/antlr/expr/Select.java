...
public class Select {
     ...
     * String-String pairs keep track of external table aliases and names.
     * Alias may be equal to the name, this means that the table has no alias.<br>
     ...
     * Columns of non-dereferenceable objects are aliases by default and need no to be added to this set.
     */
    private final Set<Entry<String, String>> columnAliases = new HashSet<>();
    /**
     ...
     */
    private boolean inFrom, lateralAllowed;
    ...
    private Select(Select parent) {
        this.schema = parent.schema;
        this.parent = parent;
        depcies = parent.depcies;
    }

    private Select findCte(String cteName) {
        return cte.contains(cteName) ? this : parent != null ? parent.findCte(cteName) : null;
    }
     ...
     */
    private Entry<String, GenericColumn> findReference(String name, String column) {
        if ((!inFrom || lateralAllowed) && namespace.containsKey(name)) {
            // column aliases imply there must be a corresponding table alias
            // so we may restrict their lookup under this condition

            // also, if we cannot dereference a name it's safe to assume all its columns are aliases
            // this saves a lookup and extra space in columnAliases
            GenericColumn dereferenced = namespace.get(name);
            if (dereferenced == null) {
                return new SimpleEntry<>(name, dereferenced);
            } else {
                boolean isColumnAlias = column != null &&
                        columnAliases.contains(new SimpleEntry<>(name, column));
                return new SimpleEntry<>(name, isColumnAlias ? null : dereferenced);
            }
        } else {
            return parent == null ? null : parent.findReference(name, column);
        }
    }
    ...
    public Set<GenericColumn> listDepcies(Select_stmtContext select) {
        select(new SelectStmt(select));
        return depcies;
    }

    private void select(SelectStmt select) {
    ...
    private void from(From_itemContext fromItem) {
        if (fromItem.LEFT_PAREN() != null && fromItem.RIGHT_PAREN() != null) {
            ...
            Value_expressionContext joinOn = fromItem.value_expression();
            if (joinOn != null) {
                // TODO do we analyze this expr at this point? later?
                // column references?
            }
        } else {
            From_primaryContext primary = fromItem.from_primary();
            if (primary == null) {
            ...
            if (table != null) {
                ...
                if (tableIds.size() > 1 || !hasCte(tableName)) {
                    String schema = QNameParser.getSchemaName(tableIds);
                    if (schema == null) {
                        schema = this.schema;
                    }
                    depcy = new GenericColumn(schema, tableName, null);
                    depcy.setType(ViewReference.TABLE);
                    depcies.add(depcy);
                }

                String aliasName;
                if (alias != null) {
                    aliasName = alias.alias.getText();
                    for (IdentifierContext columnAlias : alias.column_alias) {
                        columnAliases.add(new SimpleEntry<>(aliasName, columnAlias.getText()));
                    }
                } else {
                    aliasName = tableName;
                }
                ...
                Function_callContext function = primary.function_call();
                if (function == null) {
                ...
                try {
                    lateralAllowed = true;
                    // TODO get this from vex analyzer
                    GenericColumn funcName = new GenericColumn(null);
                    String funcAlias = primary.alias == null ? funcName.table :
