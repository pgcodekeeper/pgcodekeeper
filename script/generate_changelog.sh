...
# Основная идея запустить утилиту, затем найти поздние теги и вырезать из вывода утилиты коммиты между этими тегами
#----------------------
#------------------------------------------------------------------------------------------
...
echo "pgCodeKeeper "$version" has been released." > ${ROOT_PATH}${change_log_folder}"/"${CHANGE_LOG_FILENAME}
...
# write filtered commits to file
...
cat ${CHANGE_LOG_FILENAME} > ${ROOT_PATH}${change_log_folder}"/"${CHANGE_LOG_FILENAME}"_original.txt"
